...
def main():
    ...
    print(
        f'<svg viewBox="{min_x - 10} {min_y - 10}  {max_x - min_x + 20} {max_y - min_y + 20}" xmlns="http://www.w3.org/2000/svg" version="1.1">'
    )
    for series in value_series:
        cubics = fit_cubics(series, max_sqr_err)
        print(
            f"fit_cubics max_sqr_err {max_sqr_err} cubics {len(cubics)}",
            file=sys.stderr,
        )
        ...
        for x, y in series:
            print(
                f'  <circle cx="{x}" cy="{y}" r="0.5" fill="darkblue" opacity="0.5" />'
            )
    print("</svg>")
